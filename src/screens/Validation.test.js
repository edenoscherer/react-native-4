...
describe("React Native 5", () => {
  ...
  beforeEach(() => {
    Permissions.askAsync.mockImplementation(() =>
      Promise.resolve({ status: permissionResponse })
    );

    AsyncStorage.getItem = jest.fn(() => Promise.resolve(getItemResponse));
    AsyncStorage.setItem = jest.fn(() => Promise.resolve());
    (() => renderer.create(<Camera ref={ref => (cameraRef = ref)} />))();
  });

  it("Should have installed the expo-camera", () => {
    expect(Object.keys(packageJSON.dependencies).includes("expo-camera")).toBe(
      true
    );
  });
  ...
  it("Should have a button wrapping the user profile image", done => {
    ...
    setTimeout(() => {
      expect(
        instance.findAllByProps({ className: "profile-image-btn" }).length
      ).toBe(1);
  ...
  it("Should have a camera element when click on profile image", done => {
    ...
    setTimeout(() => {
      expect(
        instance.findAllByProps({ className: "camera-container" }).length
      ).toBe(0);
      instance
        .findAllByProps({ className: "profile-image-btn" })[0]
        .props.onPress();
  ...
  it("Should show the taked photo as the user photo", done => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        window.camera.takePictureAsync = jest.fn(() =>
          Promise.resolve({ base64: getItemResponse })
        );

        const spy2 = jest.spyOn(window.camera, "takePictureAsync");

        expect(
          instance.findAllByProps({ className: "camera-shot" }).length
        ).toBe(1);

        instance
          .findAllByProps({ className: "camera-shot" })[0]
          .props.onPress();
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            expect(
              instance.findByProps({ className: "profile-image" }).props.source
                .uri
            ).toBe("data:image/jpg;base64,photo");
            done();
          }, 600);
        }, 600);
      }, 600);
  ...
  it("Should have a close button when camera is active", done => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        instance
          .findAllByProps({ className: "camera-close" })[0]
          .props.onPress();
        expect(
          instance.findAllByProps({ className: "camera-container" }).length
        ).toBe(2);
  ...
  it("Should hide the status bar when camera is active", done => {
    ...
    setTimeout(() => {
      expect(instance.findAllByProps({ className: "status-bar" }).length).toBe(
        0
      );
      instance
        .findAllByProps({ className: "profile-image-btn" })[0]
      ...
      setTimeout(() => {
        expect(
          instance.findAllByProps({ className: "status-bar" }).length
        ).toBe(1);
        expect(
          instance.findAllByProps({ className: "status-bar" })[0].props.hidden
        ).toBe(true);
